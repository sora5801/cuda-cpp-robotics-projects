# ===========================================================================
# run_demo.ps1 — one-command demo for 34.03 (Ergodic control: spectral multiscale coverage (FFT-based — very GPU-friendly))
#
...
#
# This script needed NO changes for the real implementation — it just builds,
# runs the ergodic-control demo, and diffs its stable output lines (banner,
# PROBLEM/SCENARIO, the five gate verdicts, ARTIFACT, RESULT) against the
# committed expected_output.txt (see the "output contract" comment in
# ../src/main.cu). Keep the two in sync if a stable line ever changes.
#
...
$Slug       = 'ergodic-control'                     # substituted by scaffold.py
...
# Keep only checkable expected lines: drop blanks and '#'-prefixed provenance
# comments (expected_output.txt's header explains what was captured and how).
