# 16.05 — Side-scan sonar mosaicking + automatic target detection

**Difficulty:** intermediate · **Domain:** 16. Locomotion — Marine & Underwater

> Catalog bullet (source of truth, verbatim): `Side-scan sonar mosaicking + automatic target detection`
>
...
1. Open [`build/side-scan-sonar-mosaicking-automatic-target.sln`](build/side-scan-sonar-mosaicking-automatic-target.sln) in Visual Studio 2026.
2. Select the `Release|x64` configuration.
3. Build (Ctrl+Shift+B). The executable lands at `build/x64/Release/side-scan-sonar-mosaicking-automatic-target.exe`.
