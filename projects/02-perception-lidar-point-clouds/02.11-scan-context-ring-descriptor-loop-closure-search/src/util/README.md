...
| [`paths.h`](paths.h) | `find_data_file()` / `resolve_out_dir()` — multi-candidate path resolution so the demo finds `data/sample/` and writes `demo/out/` no matter which build layout launched it. | …you meet `main.cu`'s data loading or artifact-writing code. |
