...
| [`paths.h`](paths.h) | `find_data_file()` / `resolve_out_dir()` — multi-candidate resolution for `data/sample/*.csv` and `demo/out/` that works whether the exe is launched via the VS build layout, the CMake layout, or an IDE debugger. | …`main.cu` loads `aligned.csv`/`drifted.csv` or writes an artifact to `demo/out/`. |
