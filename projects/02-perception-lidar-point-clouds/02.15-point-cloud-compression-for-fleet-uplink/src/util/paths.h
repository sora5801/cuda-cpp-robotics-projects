...
// Role in the project
// -------------------
// Host-only helpers that answer two questions this project asks:
//
//   1) "Where is data/sample/<file>?"   -> find_data_file()
//   2) "Where do I write demo/out/…?"   -> resolve_out_dir()
//
// main.cu loads two committed clouds (structured_map.bin, pathological_cube.bin)
// and writes several artifacts (rd_curve.csv, occupancy_histogram.csv, three
// PGM top-view renders, gates_metrics.csv) — this header is included from the
// very first line of real work.
//
...
//   filename — the file's name inside data/sample/, e.g. "structured_map.bin".
