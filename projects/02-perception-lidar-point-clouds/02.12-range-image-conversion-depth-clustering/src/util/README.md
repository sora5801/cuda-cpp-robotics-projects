...
| [`paths.h`](paths.h) | `find_data_file()` / `resolve_out_dir()` — multi-candidate resolution of `data/sample/` inputs and the `demo/out/` artifact directory, so the same exe works from the VS layout, the run_demo scripts, and the optional CMake build (ratified from project 12.01's fix). Also documents the repo's **no-`<filesystem>`-under-nvcc** rule. | …main.cu loads `range_image_scene.bin` or writes a PGM/PPM/CSV artifact. |
