...
//
// main.cu uses both: find_data_file() locates the committed
// range_image_scene.bin sample, and resolve_out_dir() picks the demo/out/
// directory the PGM/PPM/CSV artifacts (range image, label maps, beta-angle
// map, gates_metrics.csv) get written into.
//
...
//   filename — the file's name inside data/sample/, e.g. "range_image_scene.bin".
