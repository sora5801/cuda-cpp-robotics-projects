...
| [`paths.h`](paths.h) | `find_data_file()` / `resolve_out_dir()` — multi-candidate resolution for `data/sample/*` inputs and the `demo/out/` artifact directory, robust to the VS build layout, a CMake build, or launch-from-IDE. | …`main.cu` loads `map.bin`/`scan_*.bin`/`cohort.csv` or writes an artifact under `demo/out/`. |
