...
| [`paths.h`](paths.h) | `find_data_file()` (locates `data/sample/lidar_scan.bin` from any launch layout) and `resolve_out_dir()` (picks/creates `demo/out/` for the PPM/CSV artifacts). | …`main.cu` loads the committed sample or writes an artifact. |
