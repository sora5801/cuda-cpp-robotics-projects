# 04.06 — Factor-graph solver: GPU sparse Cholesky + Schur complement (mini-GTSAM)

**Difficulty:** intermediate · **Domain:** 4. Sensor Fusion & State Estimation

> Catalog bullet (source of truth, verbatim): `Factor-graph solver: GPU sparse Cholesky + Schur complement (mini-GTSAM)`
>
...
1. Open [`build/factor-graph-solver.sln`](build/factor-graph-solver.sln) in Visual Studio 2026.
2. Select the `Release|x64` configuration.
3. Build (Ctrl+Shift+B). The executable lands at `build/x64/Release/factor-graph-solver.exe`.
