...
| [`paths.h`](paths.h) | `find_data_file()` / `resolve_out_dir()` — locate `data/sample/*` and `demo/out/` reliably across the VS/CMake build layouts and every launch method (IDE, `run_demo.*`). | …`main.cu` loads `scene.pgm`/`truth.csv` or writes an artifact. |
