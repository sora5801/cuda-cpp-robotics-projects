# ===========================================================================
# run_demo.ps1 — one-command demo for 01.19 (Structured-light decoding (Gray code, phase shift) for 3D scanners)
#
...
#
# STATUS: unmodified from the template — the generic build+run+diff shape
# needed no project-specific changes. Keep expected_output.txt in sync with
# the stable lines ../src/main.cu prints (see that file's "output contract"
# comment) whenever the pipeline's stable output changes.
#
...
$Slug       = 'structured-light-decoding-for-3d-scanners'                     # substituted by scaffold.py
...
# (expected_output.txt's provenance/format header lines are such comments).
