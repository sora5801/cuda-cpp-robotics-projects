# ===========================================================================
# run_demo.ps1 — one-command demo for 01.12 (Visual servoing: image-Jacobian control loop entirely on GPU)
#
...
#
# 01.12: the real implementation is in; this script is unchanged from the
# template. expected_output.txt carries the real stable lines — the output
# contract lives at the top of ../src/main.cu; change the two together. The
# demo writes demo/out/{image_plane_traces,basin_map}.ppm and three CSVs
# (git-ignored) — open the PPMs with any image viewer that reads binary
# PPM (P6), or convert with any tool (e.g. `magick convert x.ppm x.png`).
#
...
$Slug       = 'visual-servoing'                     # substituted by scaffold.py
...
# (every annotation line at the top of expected_output.txt is such a comment).
