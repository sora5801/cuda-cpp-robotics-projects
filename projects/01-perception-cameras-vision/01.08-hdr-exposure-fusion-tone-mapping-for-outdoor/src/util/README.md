...
| [`paths.h`](paths.h) | `find_data_file()` / `resolve_out_dir()` — multi-candidate path resolution so the demo finds `data/sample/` and writes `demo/out/` correctly whether launched from the VS build, `run_demo.ps1`, or the CMake path. | …`main.cu` loads the exposure bracket or writes an artifact. |
