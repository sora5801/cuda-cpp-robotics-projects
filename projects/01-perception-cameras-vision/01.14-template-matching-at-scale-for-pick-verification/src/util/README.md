...
| [`paths.h`](paths.h) | `find_data_file()` / `resolve_out_dir()` — multi-candidate resolution for `data/sample/*` inputs and the `demo/out/` artifact directory, robust to VS/CMake/IDE launch-directory differences. | …`main.cu` loads `tray.pgm`/`templates.pgm`/`truth.csv` or writes an artifact. |
