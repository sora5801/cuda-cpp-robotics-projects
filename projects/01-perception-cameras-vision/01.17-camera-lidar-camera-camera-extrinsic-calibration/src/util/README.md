...
| [`paths.h`](paths.h) | `find_data_file()` (locates `data/sample/*` across the VS/CMake/IDE-debugger launch layouts) and `resolve_out_dir()` (same for `demo/out/`). This project uses both — four `data/sample/` inputs and four `demo/out/` artifacts. | …you meet `main.cu`'s data-loading and artifact-writing code. |
