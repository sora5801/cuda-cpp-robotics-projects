# ===========================================================================
# run_demo.ps1 — one-command demo for 31.01 (Hamilton-Jacobi reachability: level-set grid solvers (stencil ops — GPU-perfect))
#
...
#
# This script needed NO changes once the real implementation (the HJ
# reachability solver) landed — it just builds, runs the exe with no
# arguments, and diffs the stable lines against expected_output.txt (see the
# "output contract" comment in ../src/main.cu for exactly which prefixes are
# stable). Keep expected_output.txt in sync if main.cu's stable lines change.
#
...
$Slug       = 'hamilton-jacobi-reachability'                     # substituted by scaffold.py
...
# (the diff-rules header at the top of expected_output.txt is such a comment).
