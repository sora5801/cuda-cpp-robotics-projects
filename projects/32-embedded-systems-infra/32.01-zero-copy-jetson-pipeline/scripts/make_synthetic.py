#!/usr/bin/env python3
"""make_synthetic.py — synthetic sample-data generator for 32.01 (Zero-copy Jetson pipeline: camera → CUDA → TensorRT → control, no CPU touches).
...
def make_saxpy_csv(n: int, seed: int, out_path: Path) -> None:
    ...
    with out_path.open("w", newline="", encoding="utf-8") as f:
        ...
        f.write("# SYNTHETIC data — generated by scripts/make_synthetic.py for project 32.01\n")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Generate the tiny synthetic sample for project 32.01 (Zero-copy Jetson pipeline: camera → CUDA → TensorRT → control, no CPU touches).")
