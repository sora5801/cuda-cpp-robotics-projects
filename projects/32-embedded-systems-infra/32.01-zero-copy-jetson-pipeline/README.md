# 32.01 — Zero-copy Jetson pipeline: camera → CUDA → TensorRT → control, no CPU touches

**Difficulty:** ★ beginner · **Domain:** 32. Embedded, Jetson & Systems Infrastructure

> Catalog bullet (source of truth, verbatim): `★ Zero-copy Jetson pipeline: camera → CUDA → TensorRT → control, no CPU touches`
>
...
1. Open [`build/zero-copy-jetson-pipeline.sln`](build/zero-copy-jetson-pipeline.sln) in Visual Studio 2026.
2. Select the `Release|x64` configuration.
3. Build (Ctrl+Shift+B). The executable lands at `build/x64/Release/zero-copy-jetson-pipeline.exe`.
