#!/usr/bin/env python3
"""make_synthetic.py — synthetic sample-data generator for 30.01 (Agriculture: fruit detection + 3D localization + ripeness; weed-vs-crop segmentation at frame rate; per-plant spray targeting; crop-row following; canopy volume from LiDAR; under-canopy navigation; yield mapping).
...
def make_saxpy_csv(n: int, seed: int, out_path: Path) -> None:
    ...
    with out_path.open("w", newline="", encoding="utf-8") as f:
        ...
        f.write("# SYNTHETIC data — generated by scripts/make_synthetic.py for project 30.01\n")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Generate the tiny synthetic sample for project 30.01 (Agriculture: fruit detection + 3D localization + ripeness; weed-vs-crop segmentation at frame rate; per-plant spray targeting; crop-row following; canopy volume from LiDAR; under-canopy navigation; yield mapping).")
