# 05.16 — Topological map extraction (generalized Voronoi via GPU brushfire)

**Difficulty:** intermediate · **Domain:** 5. SLAM, Mapping & Localization

> Catalog bullet (source of truth, verbatim): `Topological map extraction (generalized Voronoi via GPU brushfire)`
>
...
1. Open [`build/topological-map-extraction.sln`](build/topological-map-extraction.sln) in Visual Studio 2026.
2. Select the `Release|x64` configuration.
3. Build (Ctrl+Shift+B). The executable lands at `build/x64/Release/topological-map-extraction.exe`.
