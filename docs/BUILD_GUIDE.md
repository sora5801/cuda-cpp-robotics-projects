...
| Intermittent **LNK4099** in **Debug** builds (*"PDB ... not found with main.cu.obj ... linking as if no debug info"*) | A race inside the CUDA 13.3 MSBuild integration: its CudaCompile items take host-side PDB settings from a synthetic internal ClCompile placeholder that doesn't reliably receive MSBuild's computed `ProgramDataBaseFileName` default. Root-caused in project 01.02, reproduced independently on 08.01 — environment-wide, Debug-only, cosmetic (the affected `.cu.obj` files just link without embedded debug symbols on the runs where it fires; Release is untouched) | Nothing — since the Phase-1 standards retrospective, every project's `.vcxproj` suppresses this one code (`/ignore:4099`, Debug `<Link>` only) with the full root-cause explanation in an XML comment. If you see it in a project that predates the fix, it is safe to ignore or copy the template's suppression block |
| `nvcc` hard **compile error** inside MSVC's `<filesystem>` headers when building a `.cu` file | nvcc 13.3's EDG front end cannot parse MSVC's `<filesystem>` implementation — root-caused in project 07.09. This is a toolchain incompatibility, not a code bug | Never include `<filesystem>` (directly or transitively) in any translation unit nvcc compiles. Use the plain `ifstream`-probe + `_mkdir`/`mkdir` pattern in the template's [`src/util/paths.h`](PROJECT_TEMPLATE/src/util/paths.h) instead — that is exactly why it exists |
